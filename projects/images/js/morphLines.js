...
window.onload = function() {
    ...
    var img0 = unescape( Parser.commands['initialFile'] ).replace('+', ' ');
    var img1 = unescape( Parser.commands['finalFile'  ] ).replace('+', ' ');

    MorphLines.pointSet.initial = [];
    MorphLines.pointSet.final   = [];

    if ( 'marker' in Parser.commands ) {
        var markerFile = Parser.commands['marker'];
